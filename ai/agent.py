...
import psutil
import time
from util.utils import Bcolors, Mlog, FileHandler
import math
import functools
import json
...
class Agent(object):
    ...
    def reset_q_table(self):
        file = FileHandler(f'{settings.OUTPUT_PATH}results-qtables/client{self.client.id}_q_table.txt', 'w+')
        file.create_file_tree().open()
        file.content = ''
        file.write().close()

    def dump_q_table(self, index=1):
        if not settings.Q_TABLE_INDEX or settings.Q_TABLE_UPDATE_EVERYSTEP:
            file = FileHandler(f'{settings.OUTPUT_PATH}results-qtables/client{self.client.id}_q_table-{settings.STRATEGY}-{settings.EXPLORATION}-acc', 'w+')
            file.create_file_tree().open()
            try:
                file.content = str(self.q_table.tolist()) + '\n'
                file.write()
            except Exception as e:
                print(f"Could not write to file {file.file_path}")
            file.close()
    ...
    def fetch_q_table(self, index, space=1):
        ...
        try:
            file = FileHandler(
                f'{settings.OUTPUT_PATH}results-qtables/client{self.client.id}_q_table-{settings.STRATEGY}{exploration}-acc', 'r')
            file.open()
        except:
            return np.random.rand(space, self.ACTION_SPACE)
        file.read()
        file.close()
        qtable = json.loads(file.content.splitlines()[0])
        print(qtable)
        return qtable
    ...
    def dump_state(self, state):
        ...
        file = FileHandler(f'{settings.OUTPUT_PATH}{settings.STATE_FILE}-{self.OBSERVATION_SPACE}.txt', 'a+')
        file.create_file_tree().open()
        file.content = f'{self.client.id}:{state}:{self.OBSERVATION_SPACE}\n'
        file.write().close()
        return state
